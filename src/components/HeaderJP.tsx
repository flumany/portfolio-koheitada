    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-nordic-white shadow-sm' : 'bg-transparent'}`}>
      <div className="container-custom py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex space-x-8 items-center">
          ...
          <Link to="/en" className="flex items-center gap-2 bg-nordic-beige px-4 py-2 rounded-md text-sm hover:bg-opacity-80 transition-all border border-nordic-gray/20">
      ...
      {isOpen && (
        <div className="md:hidden bg-nordic-white py-4 animate-fade-in">
          <div className="container-custom flex flex-col space-y-4">
            ...
            <Link to="/en" className="flex items-center gap-2 bg-nordic-beige px-4 py-2 rounded-md text-sm hover:bg-opacity-80 transition-all w-fit border border-nordic-gray/20">
              <span className="text-xs bg-white px-2 py-1 rounded text-nordic-dark font-medium">日本語</span>
              <span>English</span>
            </Link>
          </div>
